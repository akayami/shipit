...
const {UspsClient} = require('../lib/usps');
...
describe("usps client", function() {
  ...
  describe("generateRequest", function() {
    ...
    return it("includes track ID in the track field request", done =>
      _xmlParser.parseString(_xmlDoc, function(err, doc) {
  ...
  return describe("integration tests", function() {
    ...
    return describe("out-for-delivery package with predicted delivery date", function() {
      ...
      return it("has an eta of September 25th", () => expect(_package.eta).to.deep.equal(new Date('2015-09-25T23:59:59')));
    });
  });
});
