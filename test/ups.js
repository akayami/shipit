...
const {UpsClient} = require('../lib/ups');
...
describe("ups client", function() {
  ...
  describe("generateRequest", function() {
    ...
    return it("includes a TrackRequest with the correct tracking number", done =>
      _xmlParser.parseString(_xmlDocs[2], function(err, doc) {
        const trackReq = doc['TrackRequest'];
  ...
  describe("requestOptions", function() {
    ...
    before(function() {
      ...
      return _options = _upsClient.requestOptions({trackingNumber: '1ZMYTRACK123', reference: 'zappos'});
    });

    after(() => _generateReqSpy.restore());

    it("creates a POST request", () => _options.method.should.equal('POST'));

    it("uses the correct URL", () => _options.uri.should.equal('https://www.ups.com/ups.app/xml/Track'));

    return it("calls generateRequest with the correct parameters", () => _generateReq.calledWith('1ZMYTRACK123', 'zappos').should.equal(true));
  });
  ...
  describe("validateResponse", function() {
    ...
    it("returns an error if xml response contains no nodes", function(done) {
      let errorReported = false;
      const emptyResponse = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>';
      return _upsClient.validateResponse(emptyResponse, function(err, resp) {
        err.should.exist;
        if (!errorReported) { done(); }
        return errorReported = true;
      });
  ...
  describe("getEta", function() {
    ...
    it("uses RescheduledDeliveryDate if ScheduledDeliveryDate is't available", function() {
    ...
    return it("calls presentTimestamp with a null if shipment is malformed", function() {
      const shipment = {'RandomCrap': ['garbage']};
      _upsClient.getEta(shipment);
      return _presentTimestamp.calledWith().should.equal(true);
    });
  });
  ...
  describe("getDestination", function() {
    ...
    it("calls presentAddress with the ship to address", function() {
    ...
    return it("calls presentAddress with a null if ship to address doesn't exist", function() {
      const shipment = {'ShipTo': [{'NoAddress': ['su blanca']}]};
      _upsClient.getDestination(shipment);
      return _presentAddress.calledWith().should.equal(true);
    });
  });
  ...
  describe("presentAddress", function() {
    ...
    it("calls presentLocation using the city, state, country and postal code", function(done) {
      const address = {
        city: 'Chicago',
        stateCode: 'IL',
        countryCode: 'US',
    ...
    return it("calls presentLocation even when address components aren't available", function() {
      const presentLocation = _presentLocationSpy.return('Nowhere in Africa');
      const address = _upsClient.presentAddress({});
      expect(address).to.equal('Nowhere in Africa');
      return presentLocation.calledWith().should.equal(true);
    });
  });
  ...
  return describe("integration tests", function() {
    ...
    describe("delivered package", function() {
      before(done =>
        fs.readFile('test/stub_data/ups_delivered.xml', 'utf8', (err, xmlDoc) =>
          _upsClient.presentResponse(xmlDoc, '1Z12345E0291980793', function(err, resp) {
      ...
      it("has a service of 2nd Day Air", () => expect(_package.service).to.equal('2nd Day Air'));
    ...
    describe("package in transit", function() {
      ...
      it("has a service of 2nd Day Air", () => expect(_package.service).to.equal('Next Day Air Saver'));
